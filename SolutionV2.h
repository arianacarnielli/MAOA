...
class SolutionV2 {
public:
	int n, l, m; // quantité de clients et nombre de pas de temps
	...
	vector<vector<vector<double>>> q; // quantité envoyé à chaque client : dépend de l'indice du client et du temps
	vector<vector<vector<bool>>> z; // booléan indiquant si un envoi est fait au client i à l'instant t
	vector < vector < vector < vector<bool>>>> x; // graphes avec les tournées de véhicules (vecteur de temps)
